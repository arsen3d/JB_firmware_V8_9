...
#define VerStr "V8.9.4" // detailed exact version of firmware (thanks Gregg!)
...
void loop() {
  ...
  if(min2nextrun>0) {
    ...
    runAllowed=1; // reset override for the next cycle (if the start was overridden, the session will end at the timer-specified time)
  }
  ...
  if(wifiSerial.available()>0) {
    ...
    while(wifiSerial.available()>0) {
      tempstr[ii]=wifiSerial.read(); // read a byte
      if(tempstr[ii]=='\n') start_data=0;
...
void setOutC() {
  ...
  if(inV_AC<V_AC_threshold) {
    ...
    if(ampcmd!=0xFF) { // if remote command is set, it will override anything else (but only in 240V connection)
      // need to deal with special cases of zero amps
      if(ampcmd<6) {
        setPilot(PWM_FULLON); // more the EVSE into the A / B state without pilot. compliant chargers will stop
      }
      outC=ampcmd;
    } else {
